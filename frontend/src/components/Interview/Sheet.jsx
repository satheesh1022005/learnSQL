...
const Sheet = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/problem-sets')  // Replace with your actual API route
      .then(response => {
  ...
  const toggleCompleted = (problemId) => {
    setProblems(prevState =>
      prevState.map(problem =>
        problem._id === problemId ? { ...problem, completionStatus: !problem.completionStatus } : problem
      )
    );
  ...
  const toggleRevision = (problemId) => {
    setProblems(prevState =>
      prevState.map(problem =>
        problem._id === problemId ? { ...problem, revisionMark: !problem.revisionMark } : problem
      )
    );
  ...
  const update = (problemId,type)=>{
    try {
        const response = axios.put(
          `http://localhost:5000/api/update-status?id=${problemId}&type=${type}`
        );
  ...
  const getCompletionPercentage = () => {
    ...
    const completedProblems = problems.filter(problem => problem.completionStatus).length;
  ...
  <span className="progress-label">Completion: {completionPercentage.toFixed(2)}%</span>
  <div className="progress-bar">
    <div
      className="progress"
      style={{
        width: `${completionPercentage}%`,
        backgroundColor: completionPercentage === 100 ? 'green' : completionPercentage >= 50 ? 'yellow' : 'red',
      }}
    >
      <span className="progress-text">{completionPercentage.toFixed(2)}%</span>
    </div>
  </div>
</div>
          ...
          <tbody>
            {problems.map((problem) => (
              <tr key={problem.title}>
                ...
                <td>{problem.topic}</td>
                <td>
                  <a href={problem.psLink} target="_blank" rel="noopener noreferrer">
                  <img src={leetcode} alt='leetcode' width='25px' height='25px'/>
                  </a>
                </td>
                <td>
                ...
                <td>
                  ...
                  <span
                    ...
                    className={problem.revisionMark ? 'revision-mark active' : 'revision-mark'}
                  >
